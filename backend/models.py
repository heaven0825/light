...
时光记 - 数据库模型定义
"""
from sqlalchemy import Column, Integer, String, DateTime, Text, Decimal, TinyInt, UniqueKey
...
class User(Base):
    ...
    vip_type = Column(TinyInt, default=0)  # 0=免费 1=年卡 2=终身
...
class Record(Base):
    ...
    type = Column(TinyInt, default=1)  # 1=生日 2=纪念日
    name = Column(String(30), nullable=False)
    group_type = Column(TinyInt, default=1)  # 1=家人 2=朋友 3=同事 4=客户
    date_type = Column(TinyInt, default=1)  # 1=公历 2=农历
    ...
    sms_remind = Column(TinyInt, default=0)  # 0=关闭 1=开启
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())
...
class Blessing(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    group_type = Column(TinyInt, nullable=False)  # 1=家人 2=朋友 3=同事 4=客户
    content = Column(Text, nullable=False)
    sort = Column(Integer, default=0)
    status = Column(TinyInt, default=1)  # 1=启用 0=禁用
...
class Order(Base):
    ...
    pay_type = Column(TinyInt, nullable=False)  # 1=年卡 2=终身会员
    amount = Column(Decimal(10, 2), nullable=False)
    pay_status = Column(TinyInt, default=0)  # 0=未支付 1=已支付
...
class SmsLog(Base):
    ...
    status = Column(TinyInt, default=1)  # 1=成功 0=失败
